...
export function Sidebar({ collapsed = false, onToggle, className }: SidebarProps) {
  ...
  // Auto-collapse on small screens when sidebar is uncontrolled
  const isControlled = typeof onToggle === "function"
  const [internalCollapsed, setInternalCollapsed] = useState(false)
  useEffect(() => {
    if (isControlled) return
    const update = () => {
      setInternalCollapsed(window.innerWidth < 1024) // collapse under lg
    }
    update()
    ...
    <>
      ...
      <div
        className={cn(
          ...
          effectiveCollapsed ? "w-0 overflow-hidden" : "w-64",
          className,
        )}
      >
        {/* Logo & Toggle */}
        <div className={cn("flex items-center justify-between p-4 border-b border-blue-500/30", effectiveCollapsed ? "flex-col space-y-2" : "")}> 
          <Link href="/dashboard" className="flex items-center justify-start group">
            <div className="flex items-center space-x-3">
              ...
              {!effectiveCollapsed && <span className="text-white font-bold text-xl group-hover:text-blue-100 transition-colors duration-200">RxMind</span>}
            </div>
          </Link>
          <Button
            variant="ghost"
            ...
            className={cn(
              ...
              effectiveCollapsed ? "mt-2" : ""
            )}
            onClick={handleToggle}
            aria-label="Toggle sidebar"
          >
            {effectiveCollapsed ? <ChevronRight className="w-5 h-5 text-white" /> : <ChevronLeft className="w-5 h-5 text-white" />}
          </Button>
        </div>
              ...
              <Link key={item.href} href={item.href}>
                <div
                  className={cn(
                    ...
                    effectiveCollapsed && "justify-center px-2 space-x-0"
                  )}
                >
                  <Icon className={cn("w-5 h-5 flex-shrink-0", effectiveCollapsed && "mx-auto")} />
        ...
        <div className="p-2 border-t border-blue-500/30">
          <div className={cn("flex items-center space-x-3 mb-4", effectiveCollapsed && "justify-center space-x-0")}>
